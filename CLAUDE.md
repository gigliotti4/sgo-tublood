...
- TypeScript en modo strict. Los tipos compartidos están en [resources/js/types/index.d.ts](resources/js/types/index.d.ts) (`User`, `Role`, `Permission`, `PageProps`, `PaginatedData`, `Cliente`, `ClienteAttachment`, `Observacion`, `ObservationProduct`, `Sector`, `Area`, `AlertaNotificacion`).
...
### Sectores vs. Áreas — dos listas distintas, a propósito
Hay **dos** taxonomías organizacionales y no hay que confundirlas (decisión del cliente, 22/7/2026):

| | `sectors` (Sector) | `areas` (Area) |
|---|---|---|
| Qué es | **Sector de gestión**: destino de una observación | **Área del organigrama**: dónde trabaja la persona ("sector original" del Excel de Tublood) |
| Origen | `SectorSeeder`, los 8 de `docs/ARQUITECTURA-SGO.md` | Se crean solas al importar el Excel de usuarios |
| FK | `observations.sector_id` | `users.area_id` y `observations.area_id` |
| Trae | tipos de incidencia (`config/incidencias.php`, clave = slug) | `dias_gestion` (plazo del SLA en días hábiles) |

Varios nombres se repiten entre las dos listas (Logística, Facturación, Depósito) con significados distintos — por eso son tablas separadas y **no** hay FK entre ellas. Consecuencia conocida: no se puede filtrar "responsables del sector de la observación" (idea del acta del 13/7); recuperarlo sería agregar un mapeo `areas.sector_id`.

Una observación tiene **las dos**: `sector_id` (elegido arriba, en Clasificación, porque filtra los tipos de incidencia) y `area_id` (elegido abajo, en Asignación, al lado del responsable). Ambos nullable y ambos editables desde el modal del listado.

⚠️ **`observations.area_id` no es de donde sale el vencimiento.** El reloj lo calcula `ObservacionObserver` con el área del **responsable asignado** (`$responsable->area->dias_gestion`), no con el área de la observación. Son datos con propósitos distintos: `area_id` registra a qué área se le asignó el caso —y queda fijo aunque después cambie el responsable o esa persona cambie de área—, mientras que el SLA sigue siempre a quien lo tiene que resolver.
...
Al crear una observación externa, `Portal\ObservacionController::avisar()` manda dos avisos **fuera de la transacción**: acuse de recibo al cliente y aviso al equipo de Calidad. Dos cosas no negociables ahí:
- Los destinatarios internos se buscan con `whereHas('roles', ...)`, **no** con el scope `User::role(...)` de Spatie: ese scope **tira `RoleDoesNotExist`** si el rol no está creado, y eso sería un 500 en un endpoint público (lo agarró un test).
...
- **Notificaciones** — `app/Notifications/Observacion*Notification.php` heredan de `ObservacionNotification` (canales `database` + `mail`, e **implementan `ShouldQueue`**: cada mail es una llamada HTTP a Resend, así que no puede colgar la request). El canal `database` alimenta la campana vía `HandleInertiaRequests` → `notificaciones.alertas`. La única que **no** hereda de la base es `ObservacionRecibidaClienteNotification`: su destinatario es un mail suelto (`Notification::route('mail', ...)`), no un `User`, así que no tiene canal `database` ni `$notifiable->name`.
- **Áreas** — ABM mínimo en `Admin/Areas/Index.vue` + `Admin\AreaController`, gateado con los permisos **`users.view`/`users.edit`** (no tiene permisos propios: es parte de la estructura de usuarios). Existe para corregir `dias_gestion` sin reimportar el Excel. `AreaController::update()` **no recalcula el slug** al renombrar, porque es la clave con la que el import reconoce el área.
...
Las **áreas** no tienen permisos propios: reusan `users.view`/`users.edit`.
