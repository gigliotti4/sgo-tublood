...
- **Todavía no está conectado a observaciones** — `observations` no tiene `sector_id`, no hay derivación entre sectores ni auto-asignación del portal a Garantía de Calidad (sigue como TODO en `Portal\ObservacionController`). Ver "Pendiente" en Estado del proyecto.
