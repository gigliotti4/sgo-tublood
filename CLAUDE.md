...
- ⚠️ `articulos.codigo_proveedor` (string del ERP, **sí** lo pisa la sync) y `articulos.proveedor_id` (FK al padrón local) **no son lo mismo** y no está confirmado que usen la misma numeración — por eso no hay backfill de uno al otro. En `Admin/Articulos/Edit.vue` el primero está rotulado "Cód. proveedor (ERP)" justamente para que no se confundan.
