...
- **Aviso de reclamos externos nuevos** — al entrar al panel, a quien sea de Garantía de Calidad (rol `garantia_calidad` **o** sector homónimo — `User::esDeCalidad()`) se le abre un modal con los reclamos del portal que todavía no vio, cada uno clickeable al detalle. No tiene tabla propia: sale de las `ObservacionExternaRecibidaNotification` no leídas, que `HandleInertiaRequests` comparte como `notificaciones.externas` (**solo** si no está la bandera de sesión `externas_avisadas`, para no interrumpir a mitad de una gestión: el aviso es del momento de entrar). Cerrarlo pega a `notificaciones.externas.vistas`, que marca leídos esos avisos y levanta la bandera; el login la limpia porque `session()->regenerate()` conserva los datos. Por eso `Portal\ObservacionController::avisar()` notifica a la **unión** de sector y rol, y no al sector con el rol de fallback: quien tiene el rol pero está cargado en otro sector también tiene que ver el modal.
