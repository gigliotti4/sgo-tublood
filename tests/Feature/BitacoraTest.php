 ...
 * observaciones a la vez), gateada con su propio permiso `bitacora.view`.
 */
class BitacoraTest extends TestCase
{
    ...
    public function test_requiere_el_permiso_bitacora_view(): void
    {
    ...
    /**
     * La sección se llamaba "Auditoría" y su permiso era `auditoria.view`. Ese
     * nombre queda libre para la sección de Auditoría que viene después, así
     * que no puede seguir abriendo esta pantalla ni por descuido.
     */
    public function test_el_permiso_viejo_de_auditoria_ya_no_da_acceso(): void
    {
    ...
    public function test_con_el_permiso_lista_entradas_de_todas_las_observaciones(): void
    {
        ...
        $this->actingAs($this->userWith('bitacora.view'))
            ->get('/bitacora')
    ...
    public function test_filtra_por_accion(): void
    {
        ...
        $this->actingAs($this->userWith('bitacora.view'))
            ->get('/bitacora?accion=responsable')
            ->assertInertia(fn ($page) => $page
                ->has('entradas.data', 1)
    ...
    {
        $user = $this->userWith('bitacora.view');
    ...
    public function test_filtra_por_sector(): void
    {
        ...
        $this->actingAs($this->userWith('bitacora.view'))
            ->get("/bitacora?sector_id={$sector->id}")
            ->assertInertia(fn ($page) => $page
                ->has('entradas.data', 1)
                ->where('entradas.data.0.observacion.numero', '0001-26')
            );
    ...
    public function test_filtra_por_rango_de_fechas(): void
    {
        ...
        $this->actingAs($this->userWith('bitacora.view'))
            ->get('/bitacora?desde='.now()->subDays(2)->toDateString())
            ->assertInertia(fn ($page) => $page
                ->has('entradas.data', 1)
                ->where('entradas.data.0.observacion.numero', '0002-26')
            );
    ...
    {
        $user = $this->userWith('bitacora.view', 'observaciones.view');
        ...
        $this->actingAs($user)
            ->get('/bitacora?q=telefónica')
            ->assertInertia(fn ($page) => $page->has('entradas.data', 1));

        $this->actingAs($user)
            ->get('/bitacora?q=0055-26')
            ->assertInertia(fn ($page) => $page->has('entradas.data', 1));

        $this->actingAs($user)
            ->get('/bitacora?q=texto-que-no-aparece')
